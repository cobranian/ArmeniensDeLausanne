...
- **`js/i18n.js`** — FR / EN / HY / RU dictionaries + a `data-i18n` runtime swap.
...
- **Events (`#evenements`) are auto-generated** from a daily snapshot of
  ...
  four languages live in the `COUNTRY_LABELS` table in `js/agenda.js`.
- **The "snapshot date" label is also auto-maintained.** The
  `events.source` line ("instantané du …" / "snapshot dated …" / "…
  դրությամբ" / "срез от …") is rewritten by `scripts/scrape-armenopole.mjs`
  (`updateSnapshotLabels`) in **`index.html`** (FR) and **`js/i18n.js`**
  (FR/EN/HY/RU) every time the dataset changes, so the date can't drift from
  ...
  `snapshot dated …<date>.`, `<year> թ. <month> <day>-ի դրությամբ` and
  `срез от …<date>.` — or update the regexes in `updateSnapshotLabels`
  to match.
- **The "Réseau" page (`ArmenianSwissNetwork/index.html`) is auto-generated**
  ...
  sentinel markers, the canton cards (`ASN:CANTONS`), the EN/HY/RU description
  dictionary lines (`ASN:EN` / `ASN:HY` / `ASN:RU`), and the hero counts
  ...
  the curated display name + FR/EN/HY/RU copy live in
  ...
  `peintres.html`, carries its **own inline FR/EN/HY/RU i18n + switcher**
  (separate from `js/i18n.js`; FR baseline from the markup, EN/HY/RU in its
  in-page `DICT`).
- Contact details (`#contact`) are placeholder/example content; unknown
  real-world facts are marked `[à compléter]` (and `[to complete]` /
  `[լրացնելու]` / `[заполнить]` in the other languages). Do not invent specific facts
...
- **i18n** is FR (default) / EN / HY / RU, swapped client-side by `js/i18n.js`.
